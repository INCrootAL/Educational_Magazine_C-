...
namespace ottb
{
    public partial class Jurnal3 : Form
    {
        public Jurnal3()
        {
            InitializeComponent();
        ...
        private void ShowList()
        {
            ...
            if (radioButton1.Checked)
                SqlCom = new OleDbCommand("SELECT * FROM Журнал3 WHERE IDподразделения2= "+npid+ " ORDER BY Дата DESC", con);
            if (radioButton2.Checked)
                SqlCom = new OleDbCommand("SELECT * FROM Журнал3 WHERE IDподразделения2= " + npid + "ORDER BY Работник", con);
            if (radioButton3.Checked)
                SqlCom = new OleDbCommand("SELECT * FROM Журнал3 WHERE IDподразделения2= " + npid + "ORDER BY Инструктор", con);
        ...
        private void ShowP()
        {
            ...
            SqlComP = new OleDbCommand("SELECT * FROM [ВидыИнструктажа]", con);
            ...
            while (dataReaderP.Read())
            {
                comboBox4.Items.Add(dataReaderP.GetValue(1));
            }
            dataReaderP.Close();
        ...
        private void Jurnal3_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void Jurnal3_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (ifcon) con.Close();
        }
        ...
        private bool IfNull()
        {
            ...
            if ((comboBox1.SelectedIndex == -1)|| (comboBox3.SelectedIndex == -1)|| (comboBox4.SelectedIndex == -1)|| (comboBox5.SelectedIndex == -1))
            {
        ...
        private void button6_Click(object sender, EventArgs e)
        {
            ...
            if (!IfNull())  //Вызов функции проверки полей на пустые значения
            {
                ...
                SqlCom1.CommandText = "INSERT INTO Журнал3 (IDжурнала, IDподразделения2, Дата, IDработника, IDдолжностиР, ДатаРождения, Вид, Причина, Инструктор, ДолжностьИнструктора, " +
                      " Работник, Должность) " +              
                      " VALUES(@a1, @a2, @a3, @a4, @a5, @a6, @a7, @a8, @a9, @a10, @a11, @a12)";
                SqlCom1.Parameters.Clear(); //Очистка параметров вызова
                SqlCom1.Parameters.AddWithValue("@a1", 3);
                SqlCom1.Parameters.AddWithValue("@a2", npid);
                SqlCom1.Parameters.AddWithValue("@a3", dats);
                SqlCom1.Parameters.AddWithValue("@a4", Convert.ToString(comboBox2.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a5", Convert.ToString(comboBox8.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a6", tdr);
                SqlCom1.Parameters.AddWithValue("@a7", Convert.ToString(comboBox3.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a8", Convert.ToString(comboBox4.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a9", Convert.ToString(comboBox5.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a10", rukd);
                SqlCom1.Parameters.AddWithValue("@a11", Convert.ToString(comboBox1.SelectedItem));
                SqlCom1.Parameters.AddWithValue("@a12", sdol);               
        ...
        private void Control()
        {
            ...
            OleDbCommand SqlCom1 = new OleDbCommand();
            SqlCom1.CommandText = "SELECT DISTINCT IDработника, Работник FROM Журнал3 WHERE IDподразделения2 = " + pid;
            SqlCom1.Connection = con;
            ...
            if (fl == 1)
            {
                ...
                if (fl2 == 1)
                {
                    ...
                    {
                        MessageBox.Show("Файл Журнал3 не найден!", "ОШИБКА ЧТЕНИЯ ФАЙЛА", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
